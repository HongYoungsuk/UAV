...
namespace irLib
{
	namespace irMath
	{
		...
		BSpline<-1, -1, -1> BSplineFitting(const MatrixX & givenPoints, int order, int N_ctrlPnt, const Real & xi, const Real & xf)
		{
			...
			return BSplineFitting(givenPoints, order, N_ctrlPnt, x_gp);
		}

		BSpline<-1, -1, -1> BSplineFitting(const MatrixX & givenPoints, int order, int N_ctrlPnt, const VectorX& x_gp)
		{
		...
		BSpline<-1, -1, -1> BSplineFittingFixedStartEnd(const MatrixX & givenPoints, int order, int N_ctrlPnt, const VectorX& x_gp)
		{
			int Dim = givenPoints.rows(); ///< dimension
			int N_gPnt = givenPoints.cols(); ///< number of given points
			int N_knot = N_ctrlPnt + order; ///< number of knots
			...
			*/
			...
			//std::cout << "Nall_T" << std::endl << Nall_T << std::endl;

			Nall_T(Nall_T.rows() - 1, Nall_T.cols() - 1) = 1; // Don't like it :(
			MatrixX N_T = Nall_T.block(3, 1, N_ctrlPnt - 6, N_gPnt - 2);
			MatrixX Q_T = givenPoints.block(0, 1, Dim, N_gPnt - 2)
				- givenPoints.col(0) * Nall_T.block(0, 1, 3, N_gPnt - 2).colwise().sum()
				- givenPoints.col(N_gPnt - 1) * Nall_T.block(N_ctrlPnt - 3, 1, 3, N_gPnt - 2).colwise().sum();

			MatrixX cp(Dim, N_ctrlPnt);
			cp.col(0) = givenPoints.col(0);
			cp.col(1) = givenPoints.col(0);
			cp.col(2) = givenPoints.col(0);
			cp.col(N_ctrlPnt - 1) = givenPoints.col(N_gPnt - 1);
			cp.col(N_ctrlPnt - 2) = givenPoints.col(N_gPnt - 1);
			cp.col(N_ctrlPnt - 3) = givenPoints.col(N_gPnt - 1);
			cp.block(0, 3, Dim, N_ctrlPnt - 6) = Q_T * N_T.transpose() *(N_T*N_T.transpose()).inverse();
