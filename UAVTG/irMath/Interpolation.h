...
namespace irLib
{
	namespace irMath
	{
		...
		*      xi & xf are initial & final value of parameter, respectively.
		*/
		...
		*	\brief  Yong Ming Li, Least Squares Approximation(March, 1997).
		*			givenPoints is m*n Matrix where m is the size of dimensions, and n is the number of control points.
		*			x_gp is parameter vector of size n, corresponding to viaPoints.
		*/
		BSpline<-1, -1, -1> BSplineFitting(const MatrixX & givenPoints, int order, int N_ctrlPnt, const VectorX& x_gp);

		/**
		*	\return B spline with least square error from given points.
		*	\brief  Velocity & acceleration are zero at the initial & final position.
		*			givenPoints is m*n Matrix where m is the size of dimensions, and n is the number of control points.
		*			x_gp is parameter vector of size n, corresponding to viaPoints.
		*/
